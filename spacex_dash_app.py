...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan 14 18:59:22 2024
...
options_pie = [{'label': 'All Sites', 'value': 'ALL'}]
for i in range(len(Sites)):
    options_pie.append({'label': Sites[i], 'value':Sites[i]})
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(x, values=x.values, names=x.keys(), 
...
def get_scatter_chart(entered_site, mass_range):
    ...
    if entered_site == 'ALL':
        ...
        y='class', 
        color = 'Launch Site', 
        title='Correlation between Payload Mass and Scuccess for all Sites')
        return fig
    else:
        data =  spacex_df[spacex_df['Launch Site'] == entered_site]
        ...
        title='orrelation between Payload Mass and Scuccess for site %s'%entered_site
        )
        return fig
